...
df=pd.read_csv('Olusturulan_Excel_Erkek.csv')
...
dvm = SVC( C=129 ,kernel = 'poly',probability=True)
...
clf=neighbors.KNeighborsClassifier(n_neighbors=59,weights='distance')  
...
rf_reg=RandomForestClassifier(n_estimators = 20,random_state=0,criterion='entropy',max_features=8,max_depth=1) 
...
r_dt = DecisionTreeClassifier(random_state=0,criterion='entropy',max_features=58) 
...
clf_voting=VotingClassifier(
                                    estimators=[('label1',rf_reg),('label3',gnb),('label2',r_dt)],voting='soft',n_jobs=1)
                            
clf_voting.fit(X_train_res,y_train_res)
y_pred_clf_voting=clf_voting.predict_proba(x_test)
                            
                            
fpr2, tpr2, threshold2 = metrics.roc_curve(y_test, y_pred_clf_voting[:,1])
roc_auc_voting = metrics.auc(fpr2, tpr2)
...
print("El ile elde edilen sonuc:")
AreaCalculate(fpr2,tpr2)

#### Grafikler
DRAW_ROCCURVE_WITH_SKLEARN_TPR_FPR_VALUES(fpr2,tpr2,'Voting')
