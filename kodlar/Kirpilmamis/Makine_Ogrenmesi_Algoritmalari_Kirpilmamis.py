...
df=pd.read_csv('Olusturulan_Excel_Kirpilmamis.csv')
...
clf_voting=VotingClassifier(
                                    estimators=[('label1',clf),('label3',r_dt),('dvm',gnb)],voting='soft',n_jobs=1)
                            
clf_voting.fit(X_train_res,y_train_res)
y_pred_clf_voting=clf_voting.predict_proba(x_test)
                            
                            
fpr, tpr, threshold = metrics.roc_curve(y_test, y_pred_clf_voting[:,1])
roc_auc_voting = metrics.auc(fpr, tpr)
...
#### Grafikler
DRAW_ROCCURVE_WITH_SKLEARN_TPR_FPR_VALUES(fpr,tpr,'Voting')
